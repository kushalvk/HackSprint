...
export default function EquipmentListView({ user, onLogout }) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await fetch('http://localhost:5000/api/profile/usernames', {
  ...
  const openEditModal = (eq) => {
    setNewEquipment({
      ...
      assignedEmployeeId: eq.assignedEmployeeId || null,
      department: eq.department || "",
      serialNumber: eq.serialNumber || "",
      technician: eq.technicianName || "",
      technicianId: eq.technicianId || null,
  ...
  const handleSaveEquipment = async () => {
    ...
    try {
      const equipmentData = {
        ...
        // Send both display name and object ids so backend can persist references
        employee: newEquipment.employee,
        assignedEmployee: newEquipment.assignedEmployeeId || undefined,
        department: newEquipment.department,
        technicianName: newEquipment.technician,
        technician: newEquipment.technicianId || undefined,
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setNewEquipment({
                          ...
                          employee: user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : ''
                        });
                      }}
                      className="w-full px-4 py-2 bg-slate-900/50 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
                    >
                      ...
                      {usersList.map(u => (
                        <option key={u._id} value={u._id}>{(u.firstName || u.username) + (u.lastName ? ` ${u.lastName}` : '')}</option>
                      ))}
                    </select>
                  ) : (
                    <input
                ...
                <div>
                  ...
                  {usersList.length > 0 ? (
                    <select
                      value={newEquipment.technicianId || ''}
                      onChange={(e) => {
                        const id = e.target.value || null;
                        const user = usersList.find(u => u._id === id);
                        setNewEquipment({
